...
func _on_boss_won_lookat():
	...
	if camera:
		...
		# Staring intensely at the player (long duration)
...
func _on_boss_decided(coords: Vector2i):
	...
	if grid:
		...
		var offset = (grid.grid_size - 1) * grid.cell_size / 2.0
		target_pos = grid.global_position + Vector3(coords.x * grid.cell_size - offset, 0.1, coords.y * grid.cell_size - offset)
...
func _process(delta):
	if look_weight > 0.01:
		# Rotate skull towards target
		# Skull's own transform might be overwritten by BoneAttachment, 
		# so intervening through Skeleton3D is safer.
		var bone_idx = skeleton.find_bone("mixamorig_Head")
		if bone_idx != -1:
			var current_pose = skeleton.get_bone_pose(bone_idx)
			
			# Calculate look rotation towards target (local space)
			...
			# Smooth transition between current animation pose and target pose
